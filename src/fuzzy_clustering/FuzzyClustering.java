...
public class FuzzyClustering {
    ...
    public void run(int clusterNumber, int iter, ArrayList<ArrayList<Float>> data){
        ...
        System.out.print("Covergence happens after "+ i+" iteratios: \n\n");
    }
    ...
    public void DisplayMatrixU() {
    	
    	//printing out the membership matrix
    ...
    public void displayClusters()
    {
        ...
        for (int i = 0; i < data.size(); i++) {
        		int j=0;
        		int idx=0;
            	int temp = clusterCount;
                while(temp>1) {
                	if (Math.round(u[i][j])>Math.round(u[i][j+1]))
                	{
                		System.out.print(data.get(i)+" C"+(idx+1));
                		
                	}
                	else if(Math.round(u[i][j])<Math.round(u[i][j+1]))
                	{
                		System.out.print(data.get(i)+" C"+((j+1)+1));
                		
                	}
                	else if(Math.round(u[i][j])==Math.round(u[i][j+1]))
                	{
                		System.out.print(data.get(i)+" C"+(idx+1)+" & C"+((j+1)+1));
